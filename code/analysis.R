...
print("Berkeley:")
print(summary(berkeley_model))
print(logLik(berkeley_model))
...
d_with_predictions = d %>%
    ...
    inner_join(berkeley_model %>% predict(type="response") %>% tidy() %>% transmute(X1=as.numeric(names), berkeley_predicted=x)) %>%
    ...
    inner_join(berkeley_model_genera %>% predict(type="response") %>% tidy() %>% transmute(X1=as.numeric(names), berkeley_predicted_genera=x)) %>%
...
d_with_predictions %>%
    ...
    select(order, adjusted_frequency, dryer_predicted, cysouw_predicted, cinque2_all_predicted, berkeley_predicted) %>%
    gather(key, frequency, -order) %>%
    mutate(key=factor(key, levels=c("adjusted_frequency", "dryer_predicted", "cysouw_predicted", "berkeley_predicted", "cinque2_all_predicted"))) %>%
    ...
    ggplot(aes(x=1, y=frequency, fill=key)) +
      ...
      scale_fill_manual(labels=c("Adjusted Frequency", "Dryer Prediction", "Cysouw Prediction", "Berkeley Prediction", "Cinque Prediction"), values=c("black", hues))

ggsave("../output/model_plots.pdf", width=10, height=5)
...
d_with_predictions %>%
    mutate(order=factor(order, levels=reorder(order, -d$adjusted_frequency))) %>%
    select(order, adjusted_frequency, dryer_predicted, cysouw_predicted, cinque2_all_predicted, berkeley_predicted) %>%
    mutate(dryer=dryer_predicted - adjusted_frequency, cysouw=cysouw_predicted - adjusted_frequency, cinque2=cinque2_all_predicted - adjusted_frequency, berkeley=berkeley_predicted - adjusted_frequency) %>%
    select(-adjusted_frequency) %>%
    gather(key, discrepancy, -order) %>%
    mutate(key=factor(key, levels=c("dryer", "cysouw", "cinque2", "berkeley")))  %>%
    filter(!is.na(key)) %>%
    ggplot(aes(x=1, y=discrepancy, fill=key)) +
      ...
      theme(axis.ticks.x=element_blank(),
            ...
            axis.title.x=element_blank(),
            legend.title=element_blank()) +
      scale_fill_manual(labels=c("Dryer", "Cysouw", "Cinque", "Berkeley"), values=hues)

ggsave("../output/model_discrepancies.pdf", width=10, height=5)

d_with_predictions %>%
    mutate(order=factor(order, levels=reorder(order, -d$adjusted_frequency))) %>%
    select(order, adjusted_frequency, dryer_predicted, cysouw_predicted, cinque2_all_predicted, berkeley_predicted) %>%
    mutate(dryer=chisq_discrepancy(adjusted_frequency, dryer_predicted), cysouw=chisq_discrepancy(adjusted_frequency, cysouw_predicted), cinque2=chisq_discrepancy(adjusted_frequency, cinque2_all_predicted), berkeley=chisq_discrepancy(adjusted_frequency, berkeley_predicted)) %>%
    select(-adjusted_frequency) %>%
    gather(key, discrepancy, -order) %>%
    mutate(key=factor(key, levels=c("dryer", "cysouw", "cinque2", "berkeley")))  %>%
    filter(!is.na(key)) %>%
...
ggsave("../output/model_discrepancies_chisq.pdf", width=10, height=5)
...
d_with_predictions %>%
    ...
    ggplot(aes(x=1, y=frequency, fill=key)) +
      ...
      theme(axis.ticks.x=element_blank(),
            ...
            legend.title=element_blank()) +
      #scale_alpha_discrete(range=c(.6, 1), guide=FALSE) +
      scale_fill_manual(labels=c("Genera", "Dryer Prediction", "Cysouw Prediction", "Cinque Prediction", "Berkeley Prediction"),
                        values=c("black", hues))

ggsave("../output/model_plots_genera.pdf", width=10, height=5)
...
d_with_predictions %>%
    ...
    mutate(dryer=dryer_predicted_genera - genera, cysouw=cysouw_predicted_genera - genera, cinque2=cinque2_all_predicted_genera - genera, berkeley=berkeley_predicted_genera - genera) %>%
    select(-genera) %>%
    gather(key, discrepancy, -order) %>%
    mutate(key=factor(key, levels=c("dryer", "cysouw", "cinque2", "berkeley")))  %>%
    filter(!is.na(key)) %>%
...
ggsave("../output/model_discrepancies_genera.pdf", width=10, height=5)

d_with_predictions %>%
    mutate(order=factor(order, levels=order[reorder(order, -d$genera)])) %>%
    select(order, genera, dryer_predicted_genera, cysouw_predicted_genera, cinque2_all_predicted_genera, berkeley_predicted_genera) %>%
    mutate(dryer=chisq_discrepancy(genera, dryer_predicted_genera), cysouw=chisq_discrepancy(genera, cysouw_predicted_genera), cinque2=chisq_discrepancy(genera, cinque2_all_predicted_genera), berkeley=chisq_discrepancy(genera, berkeley_predicted_genera)) %>%
    select(-genera) %>%
    gather(key, discrepancy, -order) %>%
    mutate(key=factor(key, levels=c("dryer", "cysouw", "cinque2", "berkeley")))  %>%
    filter(!is.na(key)) %>%
...
ggsave("../output/model_discrepancies_chisq_genera.pdf", width=10, height=5)
...
regression_table_figure(berkeley_model, "adjusted_frequency", "berkeley_regression_af.pdf")
...
regression_table_figure(berkeley_model_genera, "adjusted_frequency", "berkeley_regression_genera_af.pdf")
